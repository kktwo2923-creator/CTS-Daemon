...
public:
    ...
    // [Fix v4.4] 修正传给 equal() 的 end 迭代器：
    //   原版用 b.end() 作为遍历终点，但 distance(a.begin(), b.end()) 是跨堆分配
    //   的指针运算 (UB)。在 a 和 b 是不同 string 对象时，结果是垃圾值，
    //   memcmp 读到任意长度，导致比较结果几乎总是错误。
    //   正确写法：用 a.end()（同一对象内部的有效指针）作为遍历终点。
    // ====================================================================
